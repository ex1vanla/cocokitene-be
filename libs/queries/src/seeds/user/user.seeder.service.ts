...
export class UserSeederService {
  ...
  async seedUser() {
    ...
    const userSeedAdminData: InsertUserDto[] = userAdminData.map((user) => ({
      ...user,
    ...
    const normallySeedData: InsertUserDto[] = userNomallyData.map((user) => ({
      ...user,
    ...
    const [userShareholderStatus, roleShareholderNomally] = await Promise.all([
      this.userStatusRepository.getUserStatusByStatusType(
        UserStatusEnum.ACTIVE,
      ),
